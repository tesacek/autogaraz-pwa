...
// Fixní spodní navigace – hlavní navigační prvek mobilní aplikace
//
// Zobrazuje 3 záložky:
// - 🏠 Home – seznam značek
// - ⭐ Oblíbené – uložená oblíbená auta
// - 🚗 Garáž – auta v garáži + porovnání
//
// Aktivní záložka je zvýrazněna zlatou barvou
// Číslo na ikoně (badge) ukazuje počet položek v oblíbených/garáži

import { useLocation, useNavigate } from 'react-router-dom'
import { Home, Star, Car } from 'lucide-react'
import { useApp } from '../context/AppContext'

// Definice záložek navigace
const NAV_ITEMS = [
  {
    id: 'home',
    label: 'Domů',
...
export function BottomNav() {
  ...
  return (
    // glass = glassmorphism efekt s backdrop blur
    // pb-safe = padding pro iOS notch/home indicator
    <nav className="fixed bottom-0 left-0 right-0 z-40 glass border-t border-garage-border shadow-nav">
      <div className="flex items-center justify-around px-2 py-2 pb-safe" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 0.5rem)' }}>
        {NAV_ITEMS.map(({ id, label, icon: Icon, path }) => {
          // Zjistíme zda je záložka aktivní
          // Pro home kontrolujeme přesnou shodu, ostatní prefixem
